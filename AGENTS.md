...
- **Scheduling dominates micro-optimisation.** Which rewrites get applied matters more than how
  fast each is applied. Never truncate a rule's match list; keep the backoff scheduler
  hegg/egg-compatible (ban by total substitution size, bans effective on later iterations).
- **Be deterministic and order-robust.** Never rely on hash-iteration or heap-internal order for
  anything semantic; saturation trajectories are cliff-sensitive near `nodeLimit`, so judge
  benchmarks by the aggregate, not single cases.
- **Nothing hashes or compares structured data inside the join's inner loop.** Intern to dense
  `Int`s up front; keep user-facing names/values at the boundaries.
- **Accumulate in a free monoid (`FMList`/`DList`), materialise once.** Strict folds over the
  list monoid are a quadratic left-nested `(++)` trap.
- **Keep per-iteration work proportional to what changed** — no whole-graph snapshots or deep
  copies per iteration; fetch on demand.
- **Measure before keeping**: baseline CSV → change → CSV + tests, and re-profile after every
  structural change (the ranking shifts). Mutation must earn its place by measurement.

Details, history, and the evidence behind each rule: `workspace/TUNE-PLAN.md`.

## Git Commits

- Write commit messages in [Conventional Commits](https://www.conventionalcommits.org/) style
  (e.g. `perf(egraph): ...`, `docs(claude): ...`, `fix: ...`).
- Add a `Co-authored-by:` trailer to attribute the collaborating model.
- **MUST NOT** append internal information to commit messages. In particular, never add a
  `Claude-Session:` footer (or any other session URL / internal metadata) — such trailers leak
  private information into published history.
